...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			...
			printf("getip:  获取本机IP地址(支持大小写:getip/GETIP/GetIP)\r\n");
			printf("setip:  设置本机IP地址(如:setip 192.168.0.30)\r\n");
			printf("        支持多种格式:setip/SETIP/SetIP/set_ip/SET_IP\r\n");
			printf("注意:IP地址必须是合法的IP地址,否则设置失败.\r\n");
			printf("系统命令支持大小写不敏感,用户函数保持大小写敏感\r\n");
			printf("USMART使用方法:\r\n");
		...
		case 6://runtime指令,设置是否显示函数执行时间
			...
			printf("\r\n"); 
			break;	    
		case 7://getip指令,获取IP地址
		case 9://get_ip指令,获取IP地址(下划线格式)
			printf("\r\n");
			// 手动清零数组，避免memset问题
			{
				int j;
			...
			{	
				if(usmart_get_ip(sfname)==0)	//获取IP地址
				{
					printf("IP Address:%s\r\n",sfname);	//打印IP地址
				}	
				else
			...
			printf("\r\n");
			break;
		case 8://setip指令,设置IP地址
		case 10://set_ip指令,设置IP地址(下划线格式)
			printf("\r\n");
			if(USMART_ENTIMX_SCAN==0)printf("\r\nError! \r\nTo EN Set IP function,Please set USMART_ENTIMX_SCAN = 1 first!\r\n");//报错
			else
			{
				if(usmart_set_ip(str)==0)	//设置IP地址
				{
...
void usmart_scan(void)
{
	...
	{					   
		...
		// // 若输入命令后无此打印：说明串口接收功能异常，需检查接收配置。
		// u8 res = USART_ReceiveData(USART1);  // 假设用USART1
...
// IP配置函数实现，与SipMsgAnalysis.c集成
u8 usmart_get_ip(u8 *ip_str)
{
	extern u32 current_device_ip;  // 引用SipMsgAnalysis.c中定义的全局IP变量
	
	if(ip_str == 0) return 1; // 参数错误
	
	// 将IP地址从u32格式转换为字符串格式 (Big Endian)
...
u8 usmart_set_ip(u8 *ip_cmd)
{
	...
	// 处理最后一部分
	if(current_part > 255) return 1;
	ip_parts[part_count++] = current_part;
	
	// 检查是否有4个部分
	if(part_count != 4) return 1;
	
	// 构建新的IP地址 (Big Endian格式)
	...
	printf("New IP set: %d.%d.%d.%d (0x%08X)\r\n", 
