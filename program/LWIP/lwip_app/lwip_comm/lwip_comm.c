...
//	lwip_comm_default_ip_set(&lwipdev);	//设置默认IP等信?
	...
	netif_set_down(&lwip_netif); //先禁用网卡
	
	netif_set_gw(&lwip_netif, &gw);        //重新设置网关地址
	netif_set_netmask(&lwip_netif, &netmask); //重新设置子网掩码
	netif_set_ipaddr(&lwip_netif, &ipaddr);    //重新设置IP地址
//  netif_set_addr(&u_sNetif, &ip_update, &Mask_update, &GW_updata);

  netif_set_up(&lwip_netif);  //启用网卡
