...
struct stRtp gsRtp;
u8 gRtpRxBuf[RTP_BUF_NUM][172];	//本应该只需要160字节，但q->payload+12编译出错
u8 gRtpTxBuf[RTP_BUF_NUM][160];
u8 gRtpRxNum;
u8 gRtpTxNum;

/*********************************************
1. 收或发200 OK(INVITE)开始发RTP
2. 收或发200 OK(bye)停发RTP
*********************************************/
void RTP_tx(void)
{	
...
//		&&(!(gsSipRxFlag.ok_bye || gsSipTxFlag.ok_bye)))
	if(gsBatFlag.offhook||gsBatFlag.ringing)
	{
		if(gsSipTxFlag.rtp ==0){
			...
			gpsT3->rtp=0;
//			I2S2_SampleRate_Set(8000);	//设置采样
			AUDIO_rec_mode();
		}
		if(gpsT3->rtp>=20){	//每20ms
			gpsT3->rtp=0;
			RTP_build();
		}			
	}
...
void RTP_build(void)
{
  ...
  *(SendMsg+11) = ((gsRtp.ssrc) & (uint8_t)0xFF);
	...
	if(gsAudio.rtp_tx_num != gRtpTxNum){
		...
		gRtpTxNum ++;
		gRtpTxNum &= (~RTP_BUF_NUM);	//注：必须是2,4,8,16...
	}
	else{
		for(j=0;j<160;j++)
		{
			*(SendMsg+12+j) =0xD5;		//避免读跑到写的前面去了
		}
	}	
